...
async function loadComparison() {
  ...
  try {
    ...
    state.page = 1;
    $('warnings').innerHTML = ['口径：加速比 = 左侧同步延迟 ÷ 右侧同步延迟；只汇总双方成功的同配置用例。反向访存未建模，带宽留空。',...data.warnings.map(esc)].join('<br>');
